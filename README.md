...
[przepisanie na dot neta będzie w drugiej części artykułu]
...
Z drugiej strony jak sie dobrze zastanowić to zrobienie automatu nie będzie zbyt trudne i większość walki będzie z k8s RBAC niż z samym mechanizmem automatycznego update. 
...
te pierwsze to czytanie info o serwisach (będę to robił via kubectl -n default get svc -o jsonpath='{.items[*].metadata.labels.auto_ingress}')
te drugie to update ingressa , nie za bardzo chciało mi się wnikać więc dałem dosyć szeroko 
